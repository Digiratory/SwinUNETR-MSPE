...
DEFAULT_RESOLUTIONS_SwinUNETRv2 = [64, 96, 128, 160, 192, 224] # only for HVSMR-2.0
DEFAULT_K = 2
...
class MSPEPatchEmbedSwin(nn.Module):
    ...
    def __init__(
        ...
        # K patching kernels with multipled sizing
        self.patch_kernels = nn.ModuleList()
        for i in range(K):
            ...
            conv = Conv[Conv.CONV, spatial_dims](
                ...
                kernel_size=k_size, # NOTE: Overlapping kernels
                stride=self.patch_size,
                padding=pad, # to preserve feature maps dims 
                bias=True,
            )
            self.patch_kernels.append(conv)
    ...
    # padding for non-divisible images
    ...
    #     """Adaptive convolution: Apply selected overlapping kernel with zero padding 
    #     that preserves spatial resolution reduction according to self.patch_size.
    ...
    #     k_size = conv_layer.kernel_size

    #     pad_tuple = []
    #     for k, s in zip(reversed(k_size), reversed(self.patch_size)):
    #         pad = max(k - s, 0)
    #         pad_tuple.extend([pad // 2, pad - pad // 2])
            
    #     if any(p > 0 for p in pad_tuple):
    #         img = F.pad(img, tuple(pad_tuple))

    #     if self.spatial_dims == 2:
    #         return F.conv2d(img, w, bias=b, stride=self.patch_size, padding=0)
    #     else:
    #         return F.conv3d(img, w, bias=b, stride=self.patch_size, padding=0)
    ...
    def forward(self, x):
        ...
        out = self.patch_kernels[kernel_idx](x) # auto-select the best kernel for current resolution
